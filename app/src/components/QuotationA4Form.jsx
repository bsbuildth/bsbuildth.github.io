  ...
  return <article className="quote-paper quote-a4-form">
    <fieldset className="quote-a4-fields" disabled={locked}>
      <header className="quote-heading quote-form-heading">
        ...
        <div><h1>ใบเสนอราคา</h1><p className="quote-revision-badge">REV. {String(revision).padStart(2, '0')}</p><label>เลขเอกสาร<input value={quote.number} maxLength="50" placeholder="เว้นว่างเพื่อสร้างเลขอัตโนมัติ" onChange={e => edit('number', e.target.value)} /></label><label>วันที่<input type="date" value={quote.date} onChange={e => edit('date', e.target.value)} /></label><label>ผู้ขาย<input value={quote.salesperson || ''} maxLength="300" onChange={e => edit('salesperson', e.target.value)} /></label></div>
      </header>
      ...
      <table className="quote-table quote-edit-table"><thead><tr><th>รายละเอียด</th><th>จำนวน</th><th>หน่วย</th><th>ราคา/หน่วย</th><th>มูลค่า</th><th className="quote-edit-only">จัดการ</th></tr></thead>
        {quote.sections.map((section, sectionIndex) => <tbody key={section.id}>
          ...
          {section.items.map((item, itemIndex) => {
            ...
            return <tr key={item.id} className="quote-form-item"><td><span className="quote-row-number">{sectionIndex + 1}.{itemIndex + 1}</span><textarea rows="2" aria-label={`รายละเอียดรายการ ${sectionIndex + 1}.${itemIndex + 1}`} value={item.description} maxLength="2000" onChange={e => itemEdit(sectionIndex, itemIndex, 'description', e.target.value)} />{item.catalogCode && <small className="quote-catalog-origin">จากคลังราคา {item.catalogCode} · REV. {String(item.catalogVersion || 1).padStart(2, '0')}</small>}<div className="quote-row-options quote-edit-only"><label><input type="checkbox" checked={item.free} onChange={e => itemEdit(sectionIndex, itemIndex, 'free', e.target.checked)} /> แถมฟรี</label><label><input type="checkbox" checked={item.included} onChange={e => itemEdit(sectionIndex, itemIndex, 'included', e.target.checked)} /> รวมยอด</label></div></td><td><input inputMode="decimal" aria-label={`จำนวน ${sectionIndex + 1}.${itemIndex + 1}`} value={item.quantity} onChange={e => itemEdit(sectionIndex, itemIndex, 'quantity', e.target.value)} /></td><td><input aria-label={`หน่วย ${sectionIndex + 1}.${itemIndex + 1}`} value={item.unit} onChange={e => itemEdit(sectionIndex, itemIndex, 'unit', e.target.value)} /></td><td><input inputMode="decimal" aria-label={`ราคาต่อหน่วย ${sectionIndex + 1}.${itemIndex + 1}`} value={item.price} onChange={e => itemEdit(sectionIndex, itemIndex, 'price', e.target.value)} /></td><td className="quote-form-amount">{calculatedItem ? money(calculatedItem.amount) : '—'}{item.free && <small>แถมฟรี</small>}{!item.included && !item.free && <small>ไม่รวมยอด</small>}</td><td className="quote-edit-only"><div className="quote-item-actions"><button type="button" onClick={() => onSaveItemToCatalog(sectionIndex, itemIndex, item.catalogItemId ? 'update' : 'new')}>{item.catalogItemId ? 'บันทึก REV.' : 'เก็บเป็นราคา'}</button>{item.catalogItemId && <button type="button" onClick={() => onSaveItemToCatalog(sectionIndex, itemIndex, 'new')}>เป็นรายการใหม่</button>}<button type="button" className="quote-icon-button quote-danger" aria-label={`ลบรายการ ${sectionIndex + 1}.${itemIndex + 1}`} onClick={() => removeItem(sectionIndex, itemIndex)}>×</button></div></td></tr>;
          })}
          <tr className="quote-edit-only quote-add-row"><td colSpan="6"><button type="button" onClick={() => addItem(sectionIndex)}>+ สร้างรายการเอง</button><button type="button" onClick={() => addCatalogItem(sectionIndex)}>＋ เพิ่มจากราคากลาง</button></td></tr>
        </tbody>)}
      </table>
