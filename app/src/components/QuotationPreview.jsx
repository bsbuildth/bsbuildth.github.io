...
export default function QuotationPreview({ quote, status = 'draft', revision = 0 }) {
  ...
  return <article className={`quote-paper reference-document quotation-document${rows.length > 15 ? ' document-long' : ''}${onePageSignatureLayout ? ' document-one-page' : ''}`}>
    ...
    <section className="reference-notes"><b>หมายเหตุ</b><p>{quote.terms || '—'}</p>{quote.warranty && <p>การรับประกัน: {quote.warranty}</p>}<h3>เงื่อนไขการชำระเงิน</h3>{(quote.bankName || quote.bankAccountName || quote.bankAccountNumber) && <p className="reference-bank"><b>ชำระผ่านบัญชี:</b> {[quote.bankName, quote.bankAccountName && `ชื่อบัญชี ${quote.bankAccountName}`, quote.bankAccountNumber && `เลขที่ ${quote.bankAccountNumber}`].filter(Boolean).join(' · ')}</p>}<div className="reference-installments">{totals.installments.map((item, index) => <p className="reference-installment" key={index}><b>{item.label.replace('งวดที่ ', 'งวด ')}</b> · {item.percent}% · {money(item.amount)} บาท<small>{item.condition}</small></p>)}</div></section>
    <div className="reference-signatures"><div className="signature-party"><p>ผู้ว่าจ้าง / ผู้อนุมัติ</p><div className="signature-slot">{quote.customerSignatureImage && <img className="signature-image" src={quote.customerSignatureImage} alt="ลายเซ็นผู้อนุมัติ" />}</div><div><span>{quote.customerSignerName || 'ผู้อนุมัติ'}</span><span>{signedDate(quote.customerSignedAt)}</span></div></div><div className="signature-party"><p>ผู้รับจ้าง / ผู้เสนอราคา</p><div className="signature-slot">{quote.sellerSignatureImage && <img className="signature-image" src={quote.sellerSignatureImage} alt="ลายเซ็นผู้เสนอราคา" />}</div><div><span>{quote.sellerSignerName || 'ผู้เสนอราคา'}</span><span>{signedDate(quote.sellerSignedAt)}</span></div></div></div>
  </article>;
}
