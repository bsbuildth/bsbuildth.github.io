...
function Brand({ quote }) {
  return <div className="document-brand">
...
export default function QuotationPreview({ quote, status = 'draft', revision = 0 }) {
  ...
  const rows = totals.sections.flatMap(section => section.items.map(item => ({ ...item, sectionTitle: section.title, sectionKind: section.kind })));
  return <article className="quote-paper reference-document quotation-document">
    ...
    <table className="quote-table reference-table"><thead><tr><th>#</th><th>รายละเอียด</th><th>จำนวน</th><th>หน่วย</th><th>ราคาต่อหน่วย</th><th>มูลค่า</th></tr></thead>
      <tbody>{rows.map((item, index) => <tr key={item.id}><td>{index + 1}</td><td><b>{item.description || '—'}</b>{item.sectionTitle && <small>{item.sectionTitle}{item.sectionKind === 'optional' ? ' - อุปกรณ์เพิ่มเติม' : ''}</small>}{!item.included && <small>ลูกค้าจัดหาเอง</small>}{item.free && <small>แถมฟรี</small>}</td><td>{item.quantity}</td><td>{item.unit}</td><td>{money(item.unitPrice)}</td><td>{money(item.amount)}</td></tr>)}</tbody>
    </table>
    <div className="reference-summary"><div className="amount-in-words">(จำนวนเงิน {money(totals.total)} บาทถ้วน)</div><div><p><span>รวมเป็นเงิน</span><b>{money(totals.total)} บาท</b></p><p><span>ภาษีมูลค่าเพิ่ม {Number(quote.vatRate ?? 0)}%</span><b>{money(totals.vat)} บาท</b></p><p><span>ราคาไม่รวมภาษีมูลค่าเพิ่ม</span><b>{money(totals.beforeVat)} บาท</b></p><p className="reference-grand"><span>จำนวนเงินรวมทั้งสิ้น</span><b>{money(totals.total)} บาท</b></p></div></div>
