  ...
  return (
    <div className="admin-dashboard container">
      ...
      <nav className={`admin-dock-panel dock-${dockPosition.side} ${dockOpen ? 'is-open' : ''}`} style={dockPanelStyle} aria-label="เมนูจัดการระบบ">
        ...
        <button className="admin-dock-row" onClick={() => { navigate('/admin/site-survey'); setDockOpen(false); }}><i>⌑</i><span>สำรวจหน้างาน</span></button>
        <button className="admin-dock-row" onClick={() => { navigate('/admin/site-survey-reports'); setDockOpen(false); }}><i>▧</i><span>รายงานสำรวจ</span></button>
      ...
      <section className="admin-workspace" style={{ display: activeTab === 'overview' ? 'block' : 'none' }}>
        ...
        <section className="admin-project-finance" aria-label="ติดตามยอดงานที่อนุมัติ">
          ...
          {approvedWork.length === 0 ? <p className="admin-empty-state">ยังไม่มีงานที่อนุมัติ เปิดใบเสนอราคาแล้วเลือก “อนุมัติงานและล็อกยอด” เพื่อเริ่มติดตามยอด</p> : <div className="admin-project-finance-list">{approvedWork.slice(0, 8).map(row => <button key={row.id} onClick={() => navigate('/admin/quotations', { state: { quotationId: row.id } })}><span><b>{row.quote?.project || row.quote?.customer || row.quote?.number}</b><small>{row.approval?.quoteNumber || row.quote?.number} · REV. {String(row.approval?.revision || row.revision || 0).padStart(2, '0')}</small></span><span><small>อนุมัติ</small><b>{money(row.approved)}</b></span><span><small>จ่ายแล้ว</small><b>{money(row.paid)}</b></span><span className={row.outstanding ? 'is-outstanding' : 'is-paid'}><small>{row.overpaid ? 'เกินยอด' : row.outstanding ? 'คงค้าง' : 'ชำระครบ'}</small><b>{money(row.overpaid || row.outstanding)}</b></span></button>)}</div>}
        </section>
