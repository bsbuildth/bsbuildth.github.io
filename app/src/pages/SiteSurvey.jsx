...
export default function SiteSurvey() {
  ...
  const save = async event => { event.preventDefault(); if (!projectName.trim() || !customer.trim()) return setMessage('กรอกชื่อลูกค้าและชื่อโครงการก่อน'); try {
    ...
    const ref = await addDoc(collection(db, 'siteSurveys'), { quoteId:'', quoteNumber:'', projectName:projectName.trim(), customer:customer.trim(), surveyDate:date, contact:contact.trim(), address:address.trim(), location, note:note.trim(), measurements:rows.map(({ files, ...row }) => ({ ...row, area:area(row) })), totalArea:total, status:'surveyed', operationId, expectedPhotoCount:uploads.length, photoStatus:uploads.length ? 'uploading' : 'saved', photos:[], createdAt:serverTimestamp(), updatedAt:serverTimestamp() });
