...
import { archiveSiteUpdate, completeSiteUpdate, createSiteUpdate, createSiteUpdateProject, deleteSiteUpdate, listSiteUpdateProjects, listSiteUpdates, markSiteUpdateFailed, uploadSiteUpdateWithFreeDrive, usesFreeDriveBridge, validateSitePhotos } from '../firebase/siteUpdates';
...
export default function SiteUpdates() {
  ...
  useEffect(() => {
    Promise.all([getAllProjects(), listSiteUpdateProjects(), listQuotes()]).then(([websiteProjects, fieldProjects, quotations]) => {
      const approved = quotations.filter(item => item.status === 'issued' && item.approval?.state === 'approved').map(item => ({
        id: `quote-${item.id}`, title: item.quote?.project || item.quote?.customer || item.quote?.number || 'งานที่อนุมัติแล้ว',
        source: 'approved-quote', quoteId: item.id, quoteNumber: item.approval?.quoteNumber || item.quote?.number || '',
        customer: item.quote?.customer || '',
      }));
      const data = [...approved, ...fieldProjects, ...websiteProjects.map(item => ({ ...item, source: 'website' }))];
      setApprovedJobs(approved); setProjects(data); setProjectId(data[0] ? String(data[0].id) : '');
  ...
  const save = async event => {
    ...
    try {
      ...
      let result;
      if (usesFreeDriveBridge) {
        setProgress('กำลังส่งรูปเข้า Drive ฟรี…');
        result = await uploadSiteUpdateWithFreeDrive(update, files, (done, total) => setProgress(`กำลังส่งรูปเข้า Drive ${done}/${total}`));
      } else {
        setProgress('กำลังเชื่อม Google Drive…');
        const token = await requestGoogleDriveAccess();
        result = await uploadSiteUpdateDirectToDrive({ updateId: update.id, projectName: project.title, updateDate: date, files, token, onProgress: (done, total) => setProgress(`กำลังส่งรูปเข้า Drive ${done}/${total}`) });
