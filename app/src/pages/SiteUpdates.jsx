...
import { archiveSiteUpdate, createSiteUpdate, createSiteUpdateProject, listSiteUpdateProjects, listSiteUpdates, markSiteUpdateFailed, requestDriveSync, uploadSiteUpdatePhotos, validateSitePhotos } from '../firebase/siteUpdates';
...
export default function SiteUpdates() {
  ...
  const save = async event => {
    ...
    try {
      ...
      const photos = await uploadSiteUpdatePhotos(update.id, files, (done, total) => setProgress(`กำลังเตรียมรูป ${done}/${total}`));
      setProgress(`เตรียมรูป ${photos.length} รูปแล้ว กำลังส่งเข้า Drive…`);
      const result = await requestDriveSync(update.id, update.operationId, photos);
  ...
  const retry = async item => {
    try { setBusy(true); setError(''); await requestDriveSync(item.id, item.operationId, item.photos || []); await reload(); }
    catch (err) { await markSiteUpdateFailed(item.id, err.message); setError(err.message); await reload(); } finally { setBusy(false); }
  };
